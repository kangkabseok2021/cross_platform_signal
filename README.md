# Cross-Platform Signal Analyzer, Optical Positioning Simulator, CT Reconstruction Engine, Electrostatic Monitor & DICOM Pipeline

Five C++ systems in one repository — sharing CMake infrastructure, GoogleTest, and a Linux + Windows CI matrix.
...
```bash
# Build all systems (including DICOM pipeline via -DBUILD_DICOM=ON)
...
| Job | Platform | Tests |
|---|---|---|
| `cpp-build-test` | ubuntu-latest + windows-latest | cmake build + 83 GoogleTests |
| `python-tests` | ubuntu-latest | 8 pytest (signal generator + DICOM integration tests) |
